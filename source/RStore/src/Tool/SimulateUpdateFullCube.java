...
public class SimulateUpdateFullCube {

	public static class UpdateCubeMapper extends
			TableMapper<ImmutableBytesWritable, FloatWritable> {
		private FloatWritable value1 = new FloatWritable();
		private FloatWritable value2 = new FloatWritable();
		...
		public void map(ImmutableBytesWritable row, Result value,
			...
			for (KeyValue kv : kvlist) {
				...
				if (isFirst) {
					List<String> keys = iterateArray(array, 1, 5);
					for (String dimensionAttr : keys) {
						row.set(Bytes.toBytes(dimensionAttr));
						value1.set(Float.parseFloat(array[6]));
						context.write(row, value1);
					}
					isFirst = false;
				} else {
					List<String> keys = iterateArray(array, 1, 5);
					for (String dimensionAttr : keys) {
						row.set(Bytes.toBytes(dimensionAttr));
						value2.set(0 - Float.parseFloat(array[6]));
						context.write(row, value2);
					}					
				}
			}
		}

		private List<String> iterateArray(String[] array, int start, int end) {
			if (start == end) {
				List<String> keys = new LinkedList<String>();
				keys.add(array[start]);
				return keys;
			} else {
				List<String> tmpKeys = iterateArray(array, start + 1, end);
				List<String> keys = new LinkedList<String>();
				for (String key : tmpKeys) {
					keys.add(key);
					keys.add(array[start] + "|" + key);
				}
				keys.add(array[start]);
				return keys;
	...
	public static class UpdateCubeReducer
			extends
		...
		public void reduce(ImmutableBytesWritable key,
			...
			put.add(Bytes.toBytes("f1"), Bytes.toBytes("c1"),
					Bytes.toBytes(Float.toString(i)));
	...
	public static void main(String[] args) throws IOException,
		...
		Job job = new Job(config, "Simulate Update Data Cube");
		...
		scan2.setCaching(500); // 1 is the default in Scan, which will be bad
								// for MapReduce jobs
		scan2.setTimeRange(0, endDate.getTime());
		scan2.setCacheBlocks(false); // don't set to true for MR jobs

		TableMapReduceUtil.initTableMapperJob("part", // input table
				scan1, UpdateCubeMapper.class, // mapper class
				ImmutableBytesWritable.class, // mapper output key
				FloatWritable.class, // mapper output value
				job);
		TableMapReduceUtil.initTableReducerJob(datacubeName, // output table
				UpdateCubeReducer.class, // reducer class
				job);
