...
public class UpdateDataCube {
	
	public static class UpdateCubeMapper extends TableMapper<ImmutableBytesWritable,FloatWritable> {
		private LongWritable ts = new LongWritable(1);
	   	...
	   	public void map(ImmutableBytesWritable row, Result value, Context context) throws IOException, InterruptedException {	        	
	        	...
	        	for(KeyValue kv: kvlist){
	        		...
	        		if(array.length < 6)
	        			context.write(row, new FloatWritable(Float.parseFloat(array[0])));	        			
	        		else{
	        			if(dimensionAttr == null){
	    	        		dimensionAttr = array[1] + "|" + array[2] + "|" + array[3] + "|" + array[4] + "|" + array[5];
	...
	public static class UpdateCubeReducer extends TableReducer<ImmutableBytesWritable, FloatWritable, ImmutableBytesWritable>  {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException, ParseException {
		...
		Job job = new Job(config,"Update Data Cube");
		...
		TableMapReduceUtil.initTableMapperJob(
			...
			scan1,
			scan2,
			UpdateCubeMapper.class,     // mapper class
