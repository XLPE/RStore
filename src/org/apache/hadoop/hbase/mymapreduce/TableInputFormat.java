...
implements Configurable {
  ...
  public static final String INPUT_TABLE1 = "hbase.mapreduce.inputtable1";
  public static final String INPUT_TABLE2 = "hbase.mapreduce.inputtable2";
 
  /** Base-64 encoded scanner. All other SCAN_ confs are ignored if this is specified.
   * See {@link TableMapReduceUtil#convertScanToString(Scan)} for more details.
   */
  public static final String SCAN1 = "hbase.mapreduce.scan1";
  public static final String SCAN2 = "hbase.mapreduce.scan2";
  ...
  public void setConf(Configuration configuration) {
    ...
    String tableName1 = conf.get(INPUT_TABLE1);
    try {
      setHTable1(new HTable(new Configuration(conf), tableName1));
    } catch (Exception e) {
    ...
    if (conf.get(SCAN1) != null) {
      try {
        scan1 = TableMapReduceUtil.convertStringToScan(conf.get(SCAN1));
      } catch (IOException e) {
      ...
      try {
        ...
        if (conf.get(SCAN_COLUMNS) != null) {
          addColumns(scan1, conf.get(SCAN_COLUMNS));
        }

        if (conf.get(SCAN_COLUMN_FAMILY) != null) {
          scan1.addFamily(Bytes.toBytes(conf.get(SCAN_COLUMN_FAMILY)));
        }

        if (conf.get(SCAN_TIMESTAMP) != null) {
          scan1.setTimeStamp(Long.parseLong(conf.get(SCAN_TIMESTAMP)));
        }
        ...
        if (conf.get(SCAN_MAXVERSIONS) != null) {
          scan1.setMaxVersions(Integer.parseInt(conf.get(SCAN_MAXVERSIONS)));
        }

        if (conf.get(SCAN_CACHEDROWS) != null) {
          scan1.setCaching(Integer.parseInt(conf.get(SCAN_CACHEDROWS)));
        }

        // false by default, full table scans generate too much BC churn
        scan1.setCacheBlocks((conf.getBoolean(SCAN_CACHEBLOCKS, false)));
      } catch (Exception e) {
          LOG.error(StringUtils.stringifyException(e));
      }
    }
    ...
    //scan2
    String tableName2 = conf.get(INPUT_TABLE2);
    try {
      setHTable2(new HTable(new Configuration(conf), tableName2));
    } catch (Exception e) {
      LOG.error(StringUtils.stringifyException(e));
    }
    ...
    if (conf.get(SCAN2) != null) {
      try {
        scan2 = TableMapReduceUtil.convertStringToScan(conf.get(SCAN2));
      } catch (IOException e) {
        LOG.error("An error occurred.", e);
      }
    } 
