...
public class ScanWildcardColumnTracker implements ColumnTracker {
  ...
  boolean hasGetQueryVersion;
  boolean hasGetCubeVersion;
  ...
  //lifeng
  public ScanWildcardColumnTracker(int minVersion, int maxVersion, long ttl, long cT, long qT) {
	    this.maxVersions = maxVersion;
	    this.minVersions = minVersion;
	    this.oldestStamp = System.currentTimeMillis() - ttl;
  ...
  public MatchCode checkColumn(byte[] bytes, int offset, int length,
	...
	//lifeng  
	if(returnTwoVersion){
		return checkColumnForTwoVersion(bytes, offset, length, timestamp, ignoreCount);
	}
  ...
  //lifeng
  public MatchCode checkColumnForTwoVersion(byte[] bytes, int offset, int length,
	      long timestamp, boolean ignoreCount) throws IOException{
	  if (columnBuffer == null) {
	      // first iteration.
	      resetBuffer(bytes, offset, length);
	      if (ignoreCount) return ScanQueryMatcher.MatchCode.INCLUDE;
	      return checkVersionForTwoVersion(timestamp);
	    }
	    int cmp = Bytes.compareTo(bytes, offset, length,
	        columnBuffer, columnOffset, columnLength);
	    if (cmp == 0) {
  ...
  private void resetBuffer(byte[] bytes, int offset, int length) {
    ...
    columnLength = length;
    currentCount = 0;
    hasGetCubeVersion = false;
    hasGetQueryVersion = false;
  }
