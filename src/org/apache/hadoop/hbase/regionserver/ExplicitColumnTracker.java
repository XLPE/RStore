...
/**
 * This class is used for the tracking and enforcement of columns and numbers of
 * versions during the course of a Get or Scan operation, when explicit column
 * qualifiers have been asked for in the query.
 * 
 ...
 * <ul>
 * <li>{@link #checkColumn} is called when a Put satisfies all other conditions
 * of the query. This method returns a
 * {@link org.apache.hadoop.hbase.regionserver.ScanQueryMatcher.MatchCode} to
...
public class ExplicitColumnTracker implements ColumnTracker {
	...
	/**
	 * Keeps track of the latest timestamp included for current column. Used to
	...
	private long cubeTimestamp;
	boolean returnTwoVersion;
	private List<Boolean> hasGetQueryVersionList;
	 ...
	 * @param maxVersions
	 *            maximum versions to return per column
	 * @param ttl
	...
	public ExplicitColumnTracker(NavigableSet<byte[]> columns, int minVersions,
		...
		// lifeng
		this.returnTwoVersion = false;
	}

	public ExplicitColumnTracker(NavigableSet<byte[]> columns, int minVersions,
			int maxVersions, long ttl, long ct, long qt) {
		this.maxVersions = maxVersions;
		this.minVersions = minVersions;
		this.oldestStamp = System.currentTimeMillis() - ttl;
		this.columns = new ArrayList<ColumnCount>(columns.size());
		...
		// lifeng
		this.queryTimestamp = qt;
		this.cubeTimestamp = ct;
		this.returnTwoVersion = true;
		//hasGetQueryVersionList = new LinkedList<Boolean>();
	 ...
	 *            KeyValue buffer
	 * @param offset
	 *            offset to the start of the qualifier
	 * @param length
	...
	public ScanQueryMatcher.MatchCode checkColumn(byte[] bytes, int offset,
		...
		if(returnTwoVersion)
			return checkColumnForTwoVersion(bytes, offset,
					length, timestamp, ignoreCount);
		do {
			...
			// Column Matches. If it is not a duplicate key, increment the
			// version count
			// and include.
			if (ret == 0) {
				if (ignoreCount)
				...
				if (count >= maxVersions
						|| (count >= minVersions && isExpired(timestamp))) {
					// Done with versions for this column
					// Note: because we are done with this column, and are
					// removing
					// it from columns, we don't do a ++this.index. The index
					// stays
					// the same but the columns have shifted within the array
					// such
					// that index now points to the next column we are
					// interested in.
					...
					if (this.columns.size() == this.index) {
						...
						// We are done with current column; advance to next
						// column
						// of interest.
			...
			if (ret > 0) {
				// The current KV is smaller than the column the
				// ExplicitColumnTracker
				// is interested in, so seek to that column of interest.
				return ScanQueryMatcher.MatchCode.SEEK_NEXT_COL;
			}

			// The current KV is bigger than the column the
			// ExplicitColumnTracker
	 ...
	 */
	//lifeng
	public ScanQueryMatcher.MatchCode checkColumnForTwoVersion(byte[] bytes,
			int offset, int length, long timestamp, boolean ignoreCount) {
		do {
			// No more columns left, we are done with this query
			if (this.columns.size() == 0) {
				return ScanQueryMatcher.MatchCode.SEEK_NEXT_ROW; // done_row
			...
			if (ret == 0) {
				...
				resetTS();
				// keep the KV if required by minversions or it is not expired,
				// yet
				if (timestamp > queryTimestamp) {
					return ScanQueryMatcher.MatchCode.SKIP; // skip to
																		// next
																		// col
				} else if (timestamp > cubeTimestamp) {
					if (hasGetQueryVersionList.get(index))
						return MatchCode.SKIP;
					else {
						hasGetQueryVersionList.set(index, true);
						setTS(timestamp);
						return ScanQueryMatcher.MatchCode.INCLUDE;
					}
					...
					this.columns.remove(this.index);
					this.hasGetQueryVersionList.remove(this.index);
					if (this.columns.size() == this.index) {
						// We have served all the requested columns.
						this.column = null;
						return ScanQueryMatcher.MatchCode.INCLUDE_AND_SEEK_NEXT_ROW;
	...
	public void reset() {
		...
		if (returnTwoVersion) {
			...
			for (int i = 0; i < columns.size(); i++) {
				hasGetQueryVersionList.add(false);
			}
		}
		//--------------------------------------------------
		resetTS();
	 ...
	 * timestamp range and we then need to indicate this information to tracker.
	 * It is required only in case of ExplicitColumnTracker.
	 * 
	 * @param bytes
	 * @param offset
	 * @param length
	...
	public void doneWithColumn(byte[] bytes, int offset, int length) {
		while (this.column != null) {
			int compare = Bytes.compareTo(column.getBuffer(),
					column.getOffset(), column.getLength(), bytes, offset,
					length);
			resetTS();
			if (compare == 0) {
				this.columns.remove(this.index);
