...
class StoreScanner implements KeyValueScanner, InternalScanner, ChangedReadersObserver {
  ...
  StoreScanner(Store store, Scan scan, final NavigableSet<byte[]> columns)
                              throws IOException {
	  //lifeng
	//LOG.info("test scan: " + this.toString() + "   " + scan.getCT() + " " + scan.getQT());
  ...
  private List<KeyValueScanner> getScanners(Scan scan,
    ...
    List<KeyValueScanner> allStoreScanners = null;
    //lifeng
    if(scan.getNumOfStoreFiles() < 0){
    	
    	allStoreScanners = this.store.getScanners(cacheBlocks, isGet, false);	
    }
    else{
    	allStoreScanners =
    	        this.store.getScanners(scan.getCT(), scan.getNumOfStoreFiles(), cacheBlocks, isGet, false);	
    }
